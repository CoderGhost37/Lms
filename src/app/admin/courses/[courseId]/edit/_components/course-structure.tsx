...
export function CourseStructure({ data }: CourseStructureProps) {
  ...
  function SortableItem({ id, data, children, className }: SortableItemProps) {
    ...
    return (
      <li
        ...
        style={style}
        {...attributes}
        className={cn('touch-none list-none', className, isDragging ? 'z-10' : '')}
      >
        {children(listeners)}
      </li>
    )
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === 'chapter') {
      ...
      const reorderedLocalChalpter = arrayMove(items, oldIndex, newIndex)

      const updatedChapterForState = reorderedLocalChalpter.map((item, index) => ({
        ...item,
        order: index + 1,
      }))
      ...
      if (courseId) {
        const chaptersToUpdate = updatedChapterForState.map((item) => ({
          id: item.id,
          position: item.order,
        }))
    ...
    if (activeType === 'lesson' && overType === 'lesson') {
      ...
      if (courseId) {
        ...
        const reorderLessonsPromise = () => reorderLessons(courseId, chapterId, lessonsToUpdate)
  ...
  function toggleChapter(chapterId: string) {
    setItems((prevItems) =>
      prevItems.map((item) => (item.id === chapterId ? { ...item, isOpen: !item.isOpen } : item))
    )
  }
